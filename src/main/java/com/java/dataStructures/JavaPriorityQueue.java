...
//i have another Student class in JavaSort class which is same package so i used it.
//class Student{
//    private int id;
    ...
    public List<Student> getStudents(List<String> events) {
        ...
        if (!events.isEmpty()) {
            for (int i = 0; i < events.size(); i++) {
                event = events.get(i).split(" ");

                if (event[0].contains("ENTER")) {
                    name = event[1];
                    cgpa = Double.parseDouble(event[2]);
                    id = Integer.parseInt(event[3]);
                    student = new Student(id, name, cgpa);
                    studentList.add(student);
                    studentList.sort(new SortStudent());
                } else if (event[0].contains("SERVED")) {
                ...
                if (studentList.isEmpty() && i == events.size() - 1) {
                    return studentList;
                }
            }
            return studentList;
